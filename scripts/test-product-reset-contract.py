...
def test_post_local_supports_business_profile_and_event_entry_paths():
    post = read('components/post-local-wizard.tsx')
    shell = read('components/app-shell.tsx')
    combined = post + shell
    for marker in [
        'business-profile-submission-pass',
        'event-submission-entry-pass',
        ...
        assert marker in combined, f'missing business profile/event entry marker {marker}'
    assert "if (submissionIntent === 'event')" in post, 'business profile submissions must not be blocked by event-only validation'
...
def test_loop_local_needs_changes_requires_actionable_reviewer_note():
    ...
    for marker in [
        ...
        'reviewerNote is required to request changes',
        "reviewerNote: 'reviewerNote: needs stronger event photo'",
    ]:
        assert marker in api_smoke, f'missing API smoke needs-changes note gate marker {marker}'
...
def test_loop_local_safe_urls_pwa_accessibility_and_smoke_cleanup():
    ...
    for marker in [
        'post-local-preview-logo',
        'post-local-preview-media',
        'onFileSelect',
        'post-local-no-logo-submit-pass',
    ]:
        assert marker in post + mobile, f'missing Post Local media preview marker {marker}'
    for marker in [
